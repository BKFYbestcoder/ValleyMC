...
import com.myuuiii.empirewandplus.Wands.WereldWand;
...
public class WandCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "wereld" -> {
                WereldWand wereldWand = (WereldWand) EmpireWandPlus.wandHashMap.get(args[0].toLowerCase());

                if (!player.hasPermission(wereldWand.getObtainPermissionName())) {
                    player.sendMessage(EmpireWandPlus.Prefix + colorText("&cYou're not allowed to use that!"));
                    return false;
                }

                player.getInventory().addItem(wereldWand.getItem());
                player.sendMessage(EmpireWandPlus.Prefix + colorText("&7You have been given an " + wereldWand.getDisplayName()));
            }
        }
